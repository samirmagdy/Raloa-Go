...
export const FAQAccordion: React.FC<FAQAccordionProps> = ({ locale, onContactSupport }) => {
  ...
  const seoKeywords = useMemo(
    () =>
      isRtl
        ? ['أسئلة رالوا', 'دعم رالوا', 'استفسارات بايو', 'نطاق مجاني']
        : ['raloa faq', 'link in bio help', 'custom domain guide', 'zero fee store faq'],
    [isRtl]
  );
  ...
  const seoJsonLd = useMemo(
    () => ({
      ...
      mainEntity: faqData.map((item) => ({
        '@type': 'Question',
        name: isRtl ? item.questionAr : item.question,
        acceptedAnswer: {
          '@type': 'Answer',
          text: isRtl ? item.answerAr : item.answer
        }
      }))
    }),
    [isRtl]
  ...
  // Section-specific SEO hook passing unique OpenGraph description, meta tags and FAQPage JSON-LD to useSEO
  usePageSEO({
    sectionId: 'faq',
    locale,
    title: isRtl
    ...
    ogImage: 'https://raloa.app/images/og-faq.png',
  ...
  return (
    <section id="faq" className="py-10 sm:py-20 md:py-28 bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800/80 transition-colors duration-200">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-5">
            ...
            <h2 className="text-[28px] sm:text-[42px] font-extrabold text-[#0F172A] dark:text-white tracking-tight leading-tight mb-4">
              {t.headline}
            </h2>
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full py-4 px-4 sm:px-6 flex items-center justify-between text-left rtl:text-right hover:bg-slate-50/80 dark:hover:bg-slate-850 transition-colors cursor-pointer select-none"
                  >
                  ...
                  {isOpen && (
                    <div
                      ...
                      className="px-4 sm:px-6 pb-5 pt-1 text-[13px] sm:text-[15px] text-slate-600 dark:text-slate-300 leading-relaxed font-normal animate-in fade-in duration-200"
                    >
